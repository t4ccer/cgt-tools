...
use cgt::{
    ...
    impl_has,
    numeric::v2f::V2f,
    short::partizan::games::col::{Col, VertexColor},
};
...
pub struct ColWindow {
    game: AccessTracker<Col<PositionedVertex, UndirectedGraph<PositionedVertex>>>,
...
impl ColWindow {
    pub fn new() -> ColWindow {
        ColWindow {
            // caterpillar C(4, 3, 4)
            game: AccessTracker::new(Col::new(UndirectedGraph::from_edges(
                &[
...
impl IsCgtWindow for TitledWindow<ColWindow> {
    ...
    fn initialize(&mut self, ctx: &GuiContext) {
        ...
        ctx.schedule_task(
            "Snort",
            crate::Task::EvalCol(crate::EvalTask {
    ...
    fn update(&mut self, update: crate::UpdateKind) {
        let graph = self.content.game.graph.map(|v| v.color);
        if let UpdateKind::ColDetails(game, details) = update {
            if graph == game.graph {
    ...
    fn draw(&mut self, ui: &Ui, ctx: &mut GuiContext) {
        ...
        ui.window(&self.title)
            ...
            .build(|| {
                ...
                ui.same_line();
                if ui.button("Clear") {
                    *self.content.game = Col::new(UndirectedGraph::empty(&[]));
                }
                ...
                match self.content.editing_mode {
                    ...
                    GraphEditingMode::TintVertexBlue => {
                        if let Some(clicked) = clicked {
                            let clicked_vertex: &mut VertexColor = self
                                .content
                                .game
                                .graph
                                .get_vertex_mut(clicked)
                                .get_inner_mut();
                            *clicked_vertex = VertexColor::TintLeft;
                        }
                    }
                    GraphEditingMode::TintVertexRed => {
                        if let Some(clicked) = clicked {
                            let clicked_vertex: &mut VertexColor = self
                                ...
                                .get_vertex_mut(clicked)
                                .get_inner_mut();
                            *clicked_vertex = VertexColor::TintRight;
                        }
                    }
                    GraphEditingMode::TintVertexNone => {
                        if let Some(clicked) = clicked {
                            let clicked_vertex: &mut VertexColor = self
                                ...
                                .get_vertex_mut(clicked)
                                .get_inner_mut();
                            *clicked_vertex = VertexColor::Empty;
                        }
                    }
